    ...
    function(ui,email,runtime,search,record,http,log,redirect,task,format) {
        ...
        function onRequest(context) {
            if (context.request.method === "GET") {
                ...
                inlineHtml +=
                    '<script type="text/javascript" charset="utf8" src="//cdn.datatables.net/rowgroup/1.1.3/js/dataTables.rowGroup.min.js"></script> ';
                inlineHtml +=
                    '<script type="text/javascript" charset="utf8" src="//cdn.datatables.net/buttons/2.0.0/js/dataTables.buttons.min.js"></script> ';
                inlineHtml +=
                ...
                inlineHtml +=
                    '<script type="text/javascript" charset="utf8" src="//cdn.datatables.net/buttons/2.0.0/js/buttons.html5.min.js"></script> ';
                inlineHtml +=
                    '<script type="text/javascript" charset="utf8" src="//cdn.datatables.net/buttons/2.0.0/js/buttons.print.min.js"></script> ';

                // Load Netsuite stylesheet and script
                inlineHtml +=
                    '<link rel="stylesheet" href="https://1048144.app.netsuite.com/core/media/media.nl?id=2060796&c=1048144&h=9ee6accfd476c9cae718&_xt=.css"/>';
                inlineHtml +=
                    '<script src="https://1048144.app.netsuite.com/core/media/media.nl?id=2060797&c=1048144&h=ef2cda20731d146b5e98&_xt=.js"></script>';
                inlineHtml +=
                    '<link type="text/css" rel="stylesheet" href="https://1048144.app.netsuite.com/core/media/media.nl?id=2090583&c=1048144&h=a0ef6ac4e28f91203dfe&_xt=.css">';
                ...
                inlineHtml +=
                    '<div class="a" style="width: 100%; background-color: #CFE0CE; padding: 20px; min-height: 100%; height: 100%;">';
                ...
                // Define alert window.
                inlineHtml +=
                    '<div class="container" style="margin-top:14px;" hidden><div id="alert" class="alert alert-danger fade in"></div></div>';

                // Define information window.
                inlineHtml +=
                    '<div class="container" hidden><p id="info" class="alert alert-info"></p></div>';
                inlineHtml += '<div style="margin-top: -40px"><br/>';

                // Buttons
                inlineHtml +=
                    '<button style="margin-left: 10px; margin-right: 5px; background-color: #FBEA51; color: #103D39; font-weight: 700; border-color: transparent; border-width: 2px; border-radius: 15px; height: 30px" type="button" id="smc" onclick="">Service Management Console</button>';
                inlineHtml +=
                    '<h1 style="font-size: 25px; font-weight: 700; color: #103D39; text-align: center">Scheduled Price Change</h1>';
                ...
                form.addField({
                    id: "custpage_price_change_zee_id",
                    label: "Zee ID",
                ...
                form.clientScriptFileId = 5500819; // 5500819
        ...
        function zeeDropdownSection(zeeid) {
            ...
            inlineQty +=
                '<div class="row col-xs-6" style="left: 25%; margin-top: 20px;">'; //col-xs-6 d-flex justify-content-center

            inlineQty += '<div class="input-group">';
            inlineQty +=
                '<span style="background-color: #379E8F; color: white;" class="input-group-addon">Franchisee</span>';
            inlineQty +=
                '<select id="zee_filter_dropdown" class="form-control" required>';
            ...
            zeesSearchResults.each(function(zeesSearchResult) {
                var zee_id = zeesSearchResult.getValue({
                    name: "internalid",
                    summmary: search.Summary.GROUP,
                });
                var zee_name = zeesSearchResult.getValue({
                    name: "companyname",
                    summmary: search.Summary.GROUP,
                });
         ...
         */
         function dataTable_custlist() {
            var inlineQty =
                "<style>table#table_preview {font-size: 12px;text-align: center;border: none;}.dataTables_wrapper {font-size: 14px;}table#table_preview th{text-align: center;} .bolded{font-weight: bold;}</style>";
            inlineQty +=
                '<table id="table_preview" class="table table-responsive table-striped customer tablesorter hide" style="width: 100%;">';
            inlineQty += '<thead style="color: white; background-color: #379E8F;">';
            inlineQty += '<tr class="text-center">';
            inlineQty += "</tr>";
            inlineQty += "</thead>";

            inlineQty += '<tbody id="result_Table" class="result-Table"></tbody>';
        ...
        function tabsSection() {
            ...
            // Tabs headersasdasda
            var inlineQty = "<style>.nav > li.active > a, .nav > li.active > a:focus, .nav > li.active > a:hover { background-color: #379E8F; color: #fff }";
            inlineQty += ".nav > li > a, .nav > li > a:focus, .nav > li > a:hover { margin-left: 5px; margin-right: 5px; border: 2px solid #379E8F; color: #379E8F; }";
            inlineQty += "</style>"; 
            inlineQty += '<div class="tabSection" style="width: 95%; margin:auto; margin-bottom: 30px; margin-top: 30px;",><ul class="nav nav-pills nav-justified" style="margin:0%; ">';
            inlineQty += '<li role="presentation" class="active"><a data-toggle="tab" href="#edit"><b>Edit</b></a></li>';
            inlineQty += '<li role="presentation" class=""><a data-toggle="tab" href="#review"><b>Review</b></a></li>';
            inlineQty += '<li role="presentation" class=""><a data-toggle="tab" href="#upload_scf"><b>Upload SCF</b></a></li>';
            inlineQty += "</ul></div>";

            // Tabs content
            inlineQty += '<div class="tab-content">';
            // if (!isFinanceRole(userRole)) {
            inlineQty += '<div role="tabpanel" class="tab-pane active" id="edit">';
            //    inlineQty += dataTablePreview('reassign');

            inlineQty += editTab("edit");

            inlineQty += "</div>";

            inlineQty += '<div role="tabpanel" class="tab-pane" id="review">';

            inlineQty += reviewTab("review");

            inlineQty += "</div>";

            inlineQty += '<div role="tabpanel" class="tab-pane" id="upload_scf">';
        ...
        function commRegSection(commReg) {
            var inlineQty =
                "<style>table#comm_reg_preview {font-size: 12px;text-align: center;border: none;}.dataTables_wrapper {font-size: 14px;}table#comm_reg_preview th{text-align: center;} .bolded{font-weight: bold;}</style>";
            ...
            inlineQty +=
                '<h4><span style="background-color: #379E8F" class="form-group label label-default col-xs-12">Commencement Register</span></h4>';
            inlineQty += "</div></div></div>";

            inlineQty +=
                '<table id="comm_reg_preview" class="table table-responsive table-striped customer tablesorter" style="width: 100%;">';
            inlineQty += '<thead style="color: white; background-color: #379E8F;">';
            inlineQty += '<tr class="text-center">';
            ...
            inlineQty += '<tbody id="result_data" class="result-data">';
            inlineQty += "</tbody>";
            inlineQty += "</table>";
            ...
            inlineQty +=
                '<div class="col-xs-4 schedule_change_section"><input type="button" id="create_new" class="form-control btn btn-success btn-xs create_new" value="New Scheduled Change" data-toggle="tooltip" data-placement="right" title="NEW SCHEDULED CHANGE" /></div>';
            inlineQty +=
                '<div class="col-xs-4 cancel_service_section"><input type="button" id="cancel_service" class="form-control btn btn-danger btn-xs cancel_service" value="Cancel Service" data-toggle="tooltip" data-placement="right" title="CANCEL SERVICE" /></div>';
            inlineQty += '<div class="col-xs-2"></div>'; // Blank Placeholder
            inlineQty += "</div>";
            inlineQty += "</div>";
         ...
         * @return  {String}    inlineQty
         */
        function editTab(selector) {
            var inlineQty =
                "<style>table#data_preview_" +
                selector +
                " {font-size: 12px;text-align: center;border: none;}.dataTables_wrapper {font-size: 14px;}table#data_preview_" +
                selector +
            ...
            inlineQty += '<div class="col-xs-12 heading1">';
            inlineQty +=
                '<h4><span style="background-color: #379E8F" class="form-group label label-default col-xs-12">Edit</span></h4>';
            inlineQty += "</div></div></div>";
            ...
            inlineQty +=
                '<div class="col-xs-6"><div class="input-group"><span style="background-color: #379E8F; color: white;" class="input-group-addon">Date Effective <span class="mandatory">*</span></span><input type="date" id="date_effective" value="" class="form-control date_effective"/></div></div>';
            // } else {
             ...
             */
            inlineQty +=
                '<div class="col-xs-6 commencementtype"><div class="input-group"><span style="background-color: #379E8F; color: white;" class="input-group-addon" id="commencementtype_text">Sale Type <span class="mandatory">*</span></span><select id="commencementtype" class="form-control commencementtype"><option></option>';
            ...
            inlineQty += '<div class="col-xs-2"></div>'; // Blank Placeholder
            inlineQty +=
                '<div class="col-xs-4 schedule_change_section"><input type="button" id="sumbit_edit" style="color: #103D39;" class="form-control btn btn-warning btn-xs sumbit_edit" value="Submit" data-toggle="tooltip" data-placement="right" title="Submit" /></div>';
            // <button style="margin-left: 10px; margin-right: 5px; background-color: #FBEA51; color: #103D39; font-weight: 700; border-color: transparent; border-width: 2px; border-radius: 15px; height: 30px" type="button" id="smc" class="">Service Management Console</button>';
            inlineQty +=
                '<div class="col-xs-4 cancel_service_section"><input type="button" id="next_customer" style="color: #103D39;" class="form-control btn btn-warning btn-xs next_customer" value="Next Customer" data-toggle="tooltip" data-placement="right" title="Next Customer" /></div>';
            inlineQty += '<div class="col-xs-2"></div>'; // Blank Placeholder
            inlineQty += "</div>";
            inlineQty += "</div>";
        ...
        function reviewTab(selector) {
             ...
             *  Review Header
             */
            inlineQty += '<div class="form-group container review_header_section">';
            inlineQty += '<div class="row">';
            inlineQty += '<div class="col-xs-12 heading1">';
            inlineQty +=
                '<h4><span style="background-color: #379E8F" class="form-group label label-default col-xs-12">REVIEW</span></h4>';
            inlineQty += "</div></div></div>";
        ...
        function uploadSCF() {
            ...
            // inlineQty += '<h4 id="primary_table_title" class="primary_table_title_a" style="font-size: 25px; font-weight: 700; color: #103D39; text-align: center">Upload SCF</h4>';

            /**
             *  Upload SCF Header
             */
            inlineQty += '<div class="form-group container scf_header_section">';
            inlineQty += '<div class="row">';
            inlineQty += '<div class="col-xs-12 heading1">';
            inlineQty +=
                '<h4><span style="background-color: #379E8F" class="form-group label label-default col-xs-12">Upload SCF</span></h4>';
            inlineQty += "</div></div></div>";
            ...
            inlineQty += '<div class="form-group container scf_header_section">';
            inlineQty += '<div class="row">';
            inlineQty += '<div class="mb-3">';
            inlineQty +=
                '<label for="formFileSm" class="form-label">Upload SCF</label>';
            inlineQty +=
                '<input class="form-control form-control-sm" id="upload_scf_file" type="file" name="scf">';
            inlineQty += "</div>";
            inlineQty += "</div></div>";
