...
define(['N/error', 'N/runtime', 'N/search', 'N/url', 'N/record', 'N/format', 'N/email', 'N/currentRecord'],
    function(error, runtime, search, url, record, format, email, currentRecord) {
        ...
        servSearch.run().each(function(res) {
            ...
            savedList.push({ id: internalid, custid: cust_id, zeeid: zee_id, servid: service_id, date: date_eff, incval: inc_price });
            return true;
        });
                ...
                for (var x = 0; x < service_name_elem.length - 1; x++) {
                    dataTable.rows().eq(0).each(function(index) {
                        ...
                        child_row.forEach(function(child) {
                            if (child.id == service_name_elem[x].value) {
                                console.log('CustID: ' + child.custid);
                                ...
                                var total_val = (format_price + parseInt(inc_amount_elem[x].value));

                                console.log('Current Invoice Price: ' + child.curr_inv_price + ' New Value: ' + total_val);
                                $('input[data-inv-price="' + child.curr_inv_price + '"]').each(function() {
                                    if ($(this).attr('id') == child.id) {
                                        $('input[data-inv-price="' + child.curr_inv_price + '"]').val(total_val);
                                    }
                                })
        ...
        function createChild(row) {
            ...
            row.data()[6].forEach(function(el) {
                // if (!isNullorEmpty(el.complete)) {
                //     complete = true;
                // }
                childSet.push([el.item, '<label id="' + el.item + '" class="services" data-servid="' + el.id + '" data-custid="' + el.custid + '" type="text">' + el.curr_inv_price + '</label>', '<label id="' + el.id + '" class="form-control increase_amount ' + el.id + '"disabled>' + financial(el.inc_price) + '</label>', ' <input id="' + el.id + '" class="form-control total_amount ' + el.id + '" placeholder="$" type="number" data-inv-price="' + el.curr_inv_price + '" data-custid="' + el.custid + '" value="' + el.tot_price + '"/>', '<input type="date" class="form-control new_date_eff new_date_eff_' + el.id + '" ' + el.date_eff + ' value="' + el.date_eff + '"/>', el.complete]);
            });
                    ...
                    if (!isNullorEmpty(savedList)) {
                        ...
                        if (savedListFiltered.length > 0) {
                            savedListFiltered.forEach(function(res) {
                                ...
                                childObject.push({ id: service_id, item: service, curr_inv_price: inv_price, inc_price: (parseInt(res.incval) - parseInt(inv_price_val)), tot_price: res.incval, date_eff: res.date, serv_price: service_price, custid: custid, complete: true });

                                return true;
                            })
                        } else {
                            childObject.push({ id: service_id, item: service, curr_inv_price: inv_price, inc_price: '', tot_price: '', date_eff: '', serv_price: service_price, custid: custid, complete: false });
                        }
                    } else {
                        childObject.push({ id: service_id, item: service, curr_inv_price: inv_price, inc_price: '', tot_price: '', date_eff: '', serv_price: service_price, custid: custid, complete: false });
                    }
        ...
        function onclick_bulkUpdate() {
            ...
            resultSet_Service.each(function(searchResult_Service) {
                ...
                service_html += '<td><select id="zee_bulk_dropdown" class="form-control selectpicker" multiple>';
                service_html += '<option></option>';
        ...
        function saveRecord(context) {
            ...
            for (var x = 0; x < inv_price_elem.length - 1; x++) {
                ...
                if (!isNullorEmpty(inc_am) && inc_am != 0) {
                    ...
                    console.log('Customer ID: ' + cust_id + ' ServiceID ' + service_id + ' Date ' + date_eff + ' Increase Amount ' + inc_am);
                    /**
                     *  Customer ID
                     *  Franchisee
                     *  Date Effective
        ...
        function selectOptions() {
            ...
            var range_filter = $('#zee_bulk_dropdown option:selected').map(function() { return $(this).val() });
            range_filter = $.makeArray(range_filter);
            $('#zee_bulk_dropdown').selectpicker('val', range_filter);
        }
