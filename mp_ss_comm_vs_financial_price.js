	...
	resultSet.forEach(function (customer, index) {
		...
		commencedServices.forEach(function (service) {
			...
			if (hasDuplicates(nsItemIds, itemId)) {
				...
				nlapiLogExecution('AUDIT', 'itemServiceName', itemServiceName);
				...
				for (var i = 1; i <= customerRec.getLineItemCount('itempricing'); i++) {
					...
					nlapiLogExecution('AUDIT', 'serviceOldPRice', serviceOldPrice);
					nlapiLogExecution('AUDIT', 'financial price', price);
				...
				for (var i = 1; i <= customerRec.getLineItemCount('itempricing'); i++) {
					if (itemId == customerRec.getLineItemValue('itempricing', 'item', i)) {
						...
						if (price != newCommPrice) { // Price Doesn't Match - Update
							nlapiLogExecution('AUDIT', 'Updating item price', itemId);
							nlapiLogExecution('AUDIT', 'Updated to price', newCommPrice);
							// update price
							customerRec.setLineItemValue('itempricing', 'price', i, newCommPrice);
							nlapiSubmitRecord(customerRec);
							break;
